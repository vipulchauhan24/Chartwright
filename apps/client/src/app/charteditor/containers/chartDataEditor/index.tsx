...
import { getNestedValue, isArray, setByPath } from '../../utils/lib';
...
function ChartDataEditor() {
  ...
  //       </div>
  //     ) : null}
  //     {chartDataOptions.map((options: any, indx: number) => {
  //       return (
  ...
  //               ) : null
  //             }
  //             panelComponent={options.inputsProps.map((props: any) => {
  //               return (
  //                 <div className="mt-2" key={props.id}>
  //                   <InputRenderer {...props} />
  //                 </div>
  ...
  if (!chartDataConfig?.options || !chartEditableFeatures?.chartEditOptions) {
    return null;
  }
